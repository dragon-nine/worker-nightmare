...
function buildSpecs(gameId: string): Spec[] {
  return [
    { key: 'icon', label: '앱 로고', desc: '512x512 PNG', accept: 'image/png', prefix: `store/${gameId}/google-play/icon/`, maxCount: 1, width: 512, height: 512 },
    { key: 'feature', label: '대표 이미지', desc: '1024x500 JPEG/PNG', accept: 'image/png,image/jpeg', prefix: `store/${gameId}/google-play/feature/`, maxCount: 1, width: 1024, height: 500 },
    { key: 'screenshots', label: '스크린샷', desc: '1080x2160 / 최소 2장', accept: 'image/png,image/jpeg', prefix: `store/${gameId}/google-play/screenshots/`, maxCount: 8, width: 1080, height: 2160 },
  ]
}
...
function StoreCategory({ spec, onBanner }: { spec: Spec; onBanner: Props['onBanner'] }) {
  ...
  const handleFileSelected = useCallback((files: File[]) => {
    ...
    // Open cropper for first file (one at a time)
