...
export class ResultScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 결과 이모지
    const emoji = this.add.text(width / 2, height * 0.3, success ? '🎉' : '💀', {
      fontSize: '80px',
    ...
    // 결과 텍스트
    this.add.text(width / 2, height * 0.45, success ? '성공!' : '실패...', {
    ...
    this.add.text(width / 2, height * 0.55, subMsg, {
    ...
    const btn = this.add.rectangle(width / 2, btnY, 220, 52, btnColor)
