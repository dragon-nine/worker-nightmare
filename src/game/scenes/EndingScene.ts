...
export class EndingScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 엔딩 텍스트
    this.add.text(width / 2, height * 0.2, '🎊', { fontSize: '64px' }).setOrigin(0.5);

    this.add.text(width / 2, height * 0.32, '퇴사 완료!', {
      fontFamily: 'sans-serif', fontSize: '48px', color: '#ffd700',
      fontStyle: 'bold',
    }).setOrigin(0.5);

    this.add.text(width / 2, height * 0.42, '오늘부로 자유의 몸입니다', {
    ...
    this.add.text(width / 2, height * 0.8, '다시 출근하기', {
